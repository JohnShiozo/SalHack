...
public class CrystalUtils2
{
        ...
        if (newState.getBlock() == Blocks.OBSIDIAN || newState.getBlock() == Blocks.BEDROCK)
        {
            if (canPlaceCrystalAt(pos, newState))
            {
                if (!crystalBlocks.contains(pos))
                {
                    ...
                    SalHack.SendMessage("Added a block at " + pos.toString());
                }
            }
            else
            {
                SalHack.SendMessage("removed a block " + pos.toString());
                crystalBlocks.remove(pos);
            }
        }
        else
    ...
    public static void onUpdate()
    {
        ...
        if (SalHackStatic.AutoCrystalBot == null || !SalHackStatic.AutoCrystalBot.isEnabled())
            return;
        ...
        for (BlockPos pos : obbyRock)
        {
            ...
            if (dist > AutoCrystalBot.PlaceRadius.getValue())
            {
                if (alreadyContains)
                    crystalBlocks.remove(pos);
                obbyRock.remove(pos);
            ...
            {
                if (AutoCrystalBot.VerifyCrystalBlocks(mc, pos))
                    crystalBlocks.add(pos);
                continue;
            }
            
            if (alreadyContains && (!canPlaceCrystalAt(pos, mc.world.getBlockState(pos)) || !AutoCrystalBot.VerifyCrystalBlocks(mc, pos)))
                crystalBlocks.remove(pos);
        }
        ...
        for (int x = playerPosFloored.getX() - flooredRadius; x <= playerPosFloored.getX() + flooredRadius; ++x)
            for (int y = playerPosFloored.getY() - flooredRadius; y <= playerPosFloored.getY() + flooredRadius; ++y)
                for (int z = playerPosFloored.getZ() - flooredRadius; z <= playerPosFloored.getZ() + flooredRadius; ++z)
                {
                    ...
                    if (state.getBlock() == Blocks.OBSIDIAN || state.getBlock() == Blocks.BEDROCK)
                    {
                        //SalHack.SendMessage("Found a new obisidan or bedrock block at " + pos.toString());
                        obbyRock.add(pos);
                    }
                }
       ...
       // if (diff > 0)
       //     SalHack.SendMessage("[3] Took " + (System.currentTimeMillis() - ms) + " ms to execute.");
    }

    public static void onEntityRemoved(Entity entity)
    {
        if (entity instanceof EntityEnderCrystal && mc.player.getDistance(entity) <= AutoCrystalBot.PlaceRadius.getValue())
        {
